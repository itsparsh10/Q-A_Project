...
export default function CustomerSegmentSalesPage({ onBackClick }: { onBackClick: () => void }) {
  ...
  const handleGenerateSalesPage = async () => {
    ...
    try {
      ...
      const requiredFields = [
        'productService', 'customerSegment', 'keyFeatures', 'segmentDemographics',
        'segmentPainPoints', 'segmentGoals', 'uniqueValue', 'socialProof', 'objections', 'callToAction'
      ];
      ...
      const requestData = {
        ...
        customer_segment: inputs.customerSegment,
        key_features_benefits: inputs.keyFeatures,
        segment_demographics: inputs.segmentDemographics,
        segment_pain_points: inputs.segmentPainPoints,
        segment_goals: inputs.segmentGoals,
      ...
      const response = await aiToolsService.generateCustomerSegmentSalesPage(requestData);
      setResults(response);
      
      // Save tool history silently
      try {
        await saveToolHistorySilent({
          toolName: 'Customer Segment Sales Page',
          toolId: 'customer-segment-sales-page',
          outputResult: response,
          prompt: JSON.stringify(requestData)
        });
        console.log('✅ Customer segment sales page saved to history');
      } catch (historyError) {
        console.warn('⚠️ Failed to save to history:', historyError);
      }
      setActiveTab('result');
    } catch (err: any) {
      console.error('Error generating customer segment sales page:', err);
  ...
  const exportResults = () => {
    ...
    link.href = url;
    link.download = 'customer-segment-sales-page.json';
    link.click();
  ...
  const renderGenerateTab = () => (
    <div className="space-y-6">
      <SectionCard title="Product & Service Information" icon={Briefcase}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <label className="block text-xs font-semibold text-blue-900" htmlFor="customerSegment">Customer Segment</label>
            <input
              type="text"
              id="customerSegment"
              ...
              onChange={handleInputChange}
              placeholder="Define your specific customer segment"
              className="w-full px-3 py-2 text-sm border border-blue-200 rounded-lg focus:ring-2 focus:ring-blue-100 focus:border-blue-400 bg-white transition-all"
            />
          </div>
        </div>
        <div className="space-y-2">
          ...
          <textarea 
            ...
            placeholder="List the main features and benefits that appeal to this segment..."
          ></textarea>
        </div>
      </SectionCard>

      <SectionCard title="Customer Segment Analysis" icon={Users}>
        <InputField
          label="Segment Demographics & Characteristics"
          id="segmentDemographics"
          value={inputs.segmentDemographics}
          onChange={handleInputChange}
          type="textarea"
          placeholder="Describe the specific demographics, psychographics, and characteristics of this customer segment"
        />
        <InputField
          label="Segment Pain Points & Challenges"
          id="segmentPainPoints"
          value={inputs.segmentPainPoints}
          onChange={handleInputChange}
          type="textarea"
          placeholder="What specific pain points, challenges, or frustrations does this segment face?"
        />
        <InputField
          label="Segment Goals & Aspirations"
          id="segmentGoals"
          value={inputs.segmentGoals}
          onChange={handleInputChange}
          type="textarea"
          placeholder="What are this segment's goals, desires, and aspirations that your product/service can help achieve?"
        />
      </SectionCard>

      <SectionCard title="Sales Page Content" icon={Footprints}>
        <p className="text-sm text-gray-600 mb-4">Create compelling sales page content that speaks directly to your customer segment's needs and motivations.</p>
        <InputField
          ...
          onChange={handleInputChange}
          type="textarea"
          placeholder="How does your product/service uniquely solve this segment's problems?"
          Icon={Search}
        />
        <InputField
          ...
          onChange={handleInputChange}
          type="textarea"
          placeholder="Any testimonials, case studies, or social proof specifically relevant to this segment"
          Icon={Lightbulb}
        />
        <InputField
          label="Common Objections & Concerns"
          id="objections"
          value={inputs.objections}
          onChange={handleInputChange}
          type="textarea"
          placeholder="What objections or concerns might this segment have about your product/service?"
          Icon={MessageSquare}
        />
        <InputField
          ...
          onChange={handleInputChange}
          type="textarea"
          placeholder="What action do you want this segment to take? How will you motivate them to act?"
          Icon={ThumbsUp}
        />
      </SectionCard>
      ...
      <div className="space-y-3 max-h-[55vh] overflow-y-auto pr-1">
        <div className="p-3 border border-blue-200 rounded-lg bg-white hover:shadow-md transition-all duration-200 cursor-pointer group flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-medium text-blue-900">B2B Sales Page</h4>
              <p className="text-xs text-blue-600">Template for business-to-business customer segments and enterprise solutions.</p>
            </div>
          </div>
        ...
        <div className="p-3 border border-blue-200 rounded-lg bg-white hover:shadow-md transition-all duration-200 cursor-pointer group flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-medium text-blue-900">B2C Sales Page</h4>
              <p className="text-xs text-blue-600">Template for business-to-consumer segments and individual customers.</p>
            </div>
          </div>
          <button className="px-3 py-1.5 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-all flex items-center gap-1">Create</button>
        </div>
        <div className="p-3 border border-blue-200 rounded-lg bg-white hover:shadow-md transition-all duration-200 cursor-pointer group flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-medium text-blue-900">Educational Segment</h4>
              <p className="text-xs text-blue-600">Template for educational institutions and learning-focused segments.</p>
            </div>
          </div>
          <button className="px-3 py-1.5 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-all flex items-center gap-1">Create</button>
        </div>
        <div className="p-3 border border-blue-200 rounded-lg bg-white hover:shadow-md transition-all duration-200 cursor-pointer group flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-medium text-blue-900">Healthcare Segment</h4>
              <p className="text-xs text-blue-600">Template for healthcare and wellness-focused customer segments.</p>
            </div>
          </div>
          <button className="px-3 py-1.5 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-all flex items-center gap-1">Create</button>
        </div>
  ...
  const renderHistoryTab = () => {
    return (
      <div className="py-4 flex flex-col items-center justify-center text-center space-y-2">
        ...
        <p className="text-sm text-blue-700 max-w-md">View your previously generated customer segment sales pages.</p>
      </div>
    );
  };
        ...
        <div className="flex flex-col h-full w-full">
          <div className="flex items-center px-8 py-6 text-white bg-gradient-to-r from-blue-800 via-blue-600 to-blue-400 relative overflow-hidden">
            ...
            <div className="flex-1 z-10">
              <h3 className="text-2xl font-extrabold tracking-tight text-white drop-shadow">Customer Segment Sales Page</h3>
              <p className="text-base text-blue-100/90 mt-1 font-medium">Create targeted sales pages for specific customer segments</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto bg-gradient-to-b from-blue-50/50 to-white">
            ...
            <div className="p-4 pt-3">
              ...
              {activeTab === 'history' && (
                <div className="py-4">
                  <History toolName="Customer Segment Sales Page" />
                </div>
              )}
                              ...
                              <div key={index} className="p-4 bg-white rounded-lg border border-green-200">
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1">
                                    <h5 className="font-medium text-green-900 mb-2">{section.section_title}</h5>
                                    <p className="text-sm text-green-700 leading-relaxed">{section.content}</p>
                                  </div>
                                </div>
                              </div>
                            ))}
